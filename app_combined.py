...
def train_cashflow_model(df, target_col=TARGET_COL):
...
def smart_planner_page():
    ...
    st.markdown(
        """
        Use this tool to **optimize this month's spending**:

        - Ensures **positive cashflow**  
        - Targets **≥ 5% of income** as surplus when feasible  
        - Adjusts only **flexible categories** (utilities, groceries, entertainment, dining out, clothing, misc, unexpected)  
        - Two modes: Gentle (≤20% flex cuts) and Aggressive (≤40% flex cuts)
        """
    )
    ...
    with col1:
        ...
        home_owner = st.selectbox("Home Ownership", [0, 1], format_func=lambda x: "Renter" if x == 0 else "Owner")
...
def trend_dashboard_page():
    st.header("📈 24-Month Spending Trend & Risk Dashboard")

    st.markdown(
        """
        This view analyzes **24 months** of family spending data and  
        uses a trained **LSTM model** (if available) to estimate the risk of  
        **negative cashflow in the next 3 months**.
        """
    )
    ...
    uploaded = st.sidebar.file_uploader(
        "Upload spending trend CSV (optional)",
        type=["csv"],
    ...
    if uploaded is not None:
        ...
        if os.path.exists(DATA_FILE_DEFAULT):
            ...
            st.sidebar.info(f"Using local file: {DATA_FILE_DEFAULT}")
        else:
            st.error(
                f"Could not find {DATA_FILE_DEFAULT}. "
                f"Please upload your CSV in the sidebar."
            )
            return
